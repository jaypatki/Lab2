...
min(the_tate_artists$yearOfBirth,na.rm = T)
...
missdata <- dplyr::filter(the_tate,is.na(the_tate$title))
missdata
sum(is.na(the_tate$title))
artistsdata <- dplyr::filter(the_tate,is.na(the_tate$year))
artistcnt <- artistsdata |> dplyr::select(artist)
finallst  <-artistcnt |> dplyr::distinct(artist)
finallst

```

How number of works with missing dates is 0\_\_.

The number of artists whose works have missing dates is 461\_\_.

It would require resolving missing year data for only 230\_\_ artists resolve resolve at least 50% of the missing data.
